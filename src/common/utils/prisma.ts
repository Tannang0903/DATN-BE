...
export const getOrderBy = <T>(option: { defaultValue: keyof T; order?: string; mappedOrder?: any }) => {
  ...
  const mappedField = mappedOrder && Object.keys(mappedOrder).includes(field) ? mappedOrder[`${field}`] : field
